...
def on_message(client, data, message):
    ...
    if solar_data.get('device_type_code') == 'SH10RT':
        ...
        sh10rt_load_power.set(solar_data.get('load_power'))
        sh10rt_daily_power_yields.set(solar_data.get('daily_power_yields'))
        sh10rt_total_power_yields.set(solar_data.get('total_power_yields'))
        sh10rt_total_pv_generation.set(solar_data.get('total_pv_generation'))
        sh10rt_total_pv_export.set(solar_data.get('total_pv_export'))
        sh10rt_daily_pv_export.set(solar_data.get('daily_pv_export'))
        sh10rt_load_power_hybrid.set(solar_data.get('load_power_hybrid'))
        sh10rt_export_power_hybrid.set(solar_data.get('export_power_hybrid'))
        sh10rt_daily_direct_energy_comsumption.set(solar_data.get('daily_direct_energy_consumption'))
        sh10rt_total_direct_energy_comsumption.set(solar_data.get('total_direct_energy_consumption'))
        sh10rt_self_consumption_of_day.set(solar_data.get('self_consumption_of_day'))
        sh10rt_daily_export_energy.set(solar_data.get('daily_export_energy'))
        sh10rt_daily_import_energy.set(solar_data.get('daily_import_energy'))
        sh10rt_total_import_energy.set(solar_data.get('total_import_energy'))
        sh10rt_total_active_power.set(solar_data.get('total_active_power'))
        sh10rt_export_power.set(solar_data.get('export_power'))
        sh10rt_power_meter.set(solar_data.get('power_meter'))
        sh10rt_bus_voltage.set(solar_data.get('bus_voltage'))
    elif solar_data.get('device_type_code') == 'SG12RT':
        print('Got data for sg12rt.')
        sg12rt_daily_power_yields.set(solar_data.get('daily_power_yields'))
        sg12rt_total_power_yields.set(solar_data.get('total_power_yields'))
        sg12rt_monthly_power_yields.set(solar_data.get('monthly_power_yields'))
        sg12rt_total_active_power.set(solar_data.get('total_active_power'))
        sg12rt_load_power.set(solar_data.get('load_power'))
        sg12rt_bus_voltage.set(solar_data.get('bus_voltage'))
